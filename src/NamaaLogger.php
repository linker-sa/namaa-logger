...
use Namaa\NamaaLogger\Jobs\HandleSendLogToElastic;
use Throwable;
...
class NamaaLogger
{
    ...
    public static function start($app)
    {
        ...
        // if (
        //     !static::runningWithinOctane($app) &&
    ...
    protected static function runningApprovedArtisanCommand($app)
    {
        return $app->runningInConsole() && !in_array(
            ...
            array_merge([
                ...
                'horizon:work',
                'horizon:supervisor',
            ], config('namaa-logger.ignoreCommands', []), config('namaa-logger.ignore_commands', []))
        );
    }
    ...
    {
        return is_null(config('namaa-logger.domain')) ||
            config('namaa-logger.domain') !== $request->getHost();
    }
    ...
    {
        if (!empty($only = config('namaa-logger.only_paths', []))) {
        ...
        return !$request->is(
            collect([
                ...
                ->merge(config('namaa-logger.ignore_paths', []))
                ->unless(is_null(config('namaa-logger.path')), function ($paths) {
                    return $paths->prepend(config('namaa-logger.path') . '*');
                })
                ->all()
        );
    }
     ...
     *
     * @param  string  $type
     * @param  \Namaa\NamaaLogger\IncomingEntry  $entry
     * @return void
     */
    protected static function record(string $type, IncomingEntry $entry)
    {
        ...
        try {
            HandleSendLogToElastic::dispatch($entry->toArray());
        } catch (\Throwable $th) {
            //throw $th;
        }
    }
     ...
     *
     * @param  \Namaa\NamaaLogger\EntryUpdate  $update
     * @return void
     */
     ...
     *
     * @param  \Namaa\NamaaLogger\Contracts\EntriesRepository  $storage
     * @return void
     */
